...
export function initialState() {
  return { schema: 1, revision: 0, initialized: false, currency: null, timezone: null, categories: [
...
export function analytics(state, filters = {}, now, zone = state.timezone || ZONE) {
  ...
  for (const t of state.transactions) {
    const day = dateKey(t.createdAt, z); running += effect(t);
  ...
  const firstDate = range.from === '0001-01-01' ? (state.transactions[0] ? dateKey(state.transactions[0].createdAt, z) : today) : range.from;
  const trackingStarted = state.transactions[0] ? dateKey(state.transactions[0].createdAt, z) : null;
