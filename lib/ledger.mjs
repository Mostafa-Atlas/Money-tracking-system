...
export const MAX_MONEY = 100_000_000; // EGP 1 million per entry, integer piastres.
...
export function money(value, allowZero = false) {
  fail(typeof value === 'string' && /^\d{1,7}(\.\d{1,2})?$/.test(value.trim()), 'Enter a valid EGP amount with up to two decimal places.');
  const [whole, fraction = ''] = value.trim().split('.');
  const cents = Number(whole) * 100 + Number(fraction.padEnd(2, '0'));
  fail(cents <= MAX_MONEY && (allowZero ? cents >= 0 : cents > 0), `Enter an amount ${allowZero ? 'from 0' : 'above 0'} and no more than 1,000,000 EGP.`);
  return cents;
}
...
export function initialState() {
  return { schema: 1, revision: 0, initialized: false, categories: [
