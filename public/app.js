...
const amount = cents => new Intl.NumberFormat('en-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(cents / 100);
const egp = cents => `${amount(cents)} EGP`;
...
function renderOpening() {
  app.innerHTML = `<main id="main" class="auth-page onboarding"><div class="auth-card"><div class="brand"><img src="/icon.svg" alt="">Pocket</div><section class="panel auth-panel"><div class="auth-mark">${icon('wallet')}</div><h1>Start with what's in hand.</h1><p>Count the physical cash you have right now. This is your starting point—you can correct it later.</p><form id="opening" class="form-grid"><label class="field"><span>Your current cash</span><div class="amount-input"><input name="amount" type="text" inputmode="decimal" placeholder="0.00" maxlength="10" required autocomplete="off" aria-describedby="opening-error"><span>EGP</span></div></label><div id="opening-error" class="inline-error" role="alert"></div><button class="btn full">Let's get started ${icon('arrow')}</button><p class="form-tip">${icon('shield')}Just cash. Your bank money stays separate.</p></form></section><p class="auth-foot"><button class="text-button" data-action="lock">${icon('lock')}Lock Pocket</button></p></div></main>`;
  $('#opening input')?.focus();
}
...
function openDetail(t) {
  formContext = { old: t };
  modalFrame(transactionTitle(t), `${types[t.type]} · ${formatDate(t.createdAt, { dateStyle: 'medium', timeStyle: 'short' })}`, `<div class="detail-amount">${t.effect >= 0 ? '+' : '−'}${amount(Math.abs(t.effect))}<small>EGP</small></div><div class="detail-row"><span>Type</span><span>${types[t.type]}</span></div>${t.category ? `<div class="detail-row"><span>Category</span><span>${escape(t.category)}</span></div>` : ''}${t.subject ? `<div class="detail-row"><span>Subject</span><span>${escape(t.subject)}</span></div>` : ''}${t.refunded ? `<div class="detail-row"><span>Already refunded</span><span>${egp(t.refunded)}</span></div>` : ''}${t.description ? `<div class="detail-note">${escape(t.description)}</div>` : ''}${t.type === 'opening' ? '<p class="caption">Use Edit balance in Settings to correct your current cash.</p>' : `<div class="detail-buttons">${t.type !== 'adjustment' ? `<button class="btn secondary" data-action="edit-record" data-id="${t.id}">${icon('edit')}Edit</button>` : ''}${t.type === 'expense' && t.refundable > 0 ? `<button class="btn secondary" data-action="refund-record" data-id="${t.id}">${icon('restore')}Refund</button>` : ''}<button class="btn danger" data-action="delete-record" data-id="${t.id}">${icon('trash')}Delete</button></div>`}`);
}
